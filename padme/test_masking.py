...
from padme.masking import MaskedAttributeError
from padme.masking import MaskedKeyError
from padme.masking import masking_proxy
from padme.tests import reality_is_broken
...
class masking_proxy_test(unittest.TestCase):

    """Tests for masking_proxy."""

    def setUp(self):
        self.real = Person(str("Bob"), 42)
        self.unmasked = proxy(self.real)
        self.masked = masking_proxy(self.real, {'age'})

    def test_dunder_dict_personality(self):
        self.assertIs(self.real.__dict__, self.unmasked.__dict__)
        self.assertIsNot(self.real.__dict__, self.masked.__dict__)

    def test_real_age_can_be_discovered_via_dunder_dict(self):
        self.assertIn('age', self.real.__dict__)
        self.assertEqual(self.real.__dict__['age'], 42)
        # XXX: repr() return value is non-deterministic but has determinist len
        self.assertEqual(
            len(repr(self.real.__dict__)),
            len(repr({str('name'): str('Bob'), str('age'): 42})))
        self.assertEqual(len(self.real.__dict__), 2)

    def test_real_age_can_be_mutated_via_dunder_dict(self):
        self.real.__dict__['age'] = 99
        self.assertIn('age', self.real.__dict__)
        self.assertEqual(self.real.__dict__['age'], 99)
        # XXX: repr() return value is non-deterministic but has determinist len
        self.assertEqual(
            len(repr(self.real.__dict__)),
            len(repr({str('name'): str('Bob'), str('age'): 99})))
        self.assertEqual(len(self.real.__dict__), 2)

    def test_real_age_can_be_deleted_via_dunder_dict(self):
        del self.real.__dict__['age']
        self.assertNotIn('age', self.real.__dict__)
        with self.assertRaisesRegex(KeyError, 'age'):
            self.real.__dict__['age']
        self.assertEqual(
            repr(self.real.__dict__), repr({str('name'): str('Bob')}))
        self.assertEqual(len(self.real.__dict__), 1)

    def test_unmasked_age_can_be_discovered_via_dunder_dict(self):
        self.assertIn('age', self.unmasked.__dict__)
        self.assertEqual(self.unmasked.__dict__['age'], 42)
        # XXX: repr() return value is non-deterministic but has determinist len
        self.assertEqual(
            len(repr(self.unmasked.__dict__)),
            len(repr({str('name'): str('Bob'), str('age'): 42})))
        self.assertEqual(len(self.unmasked.__dict__), 2)

    def test_unmasked_age_can_be_mutated_via_dunder_dict(self):
        self.unmasked.__dict__['age'] = 99
        self.assertIn('age', self.unmasked.__dict__)
        self.assertEqual(self.unmasked.__dict__['age'], 99)
        # XXX: repr() return value is non-deterministic but has determinist len
        self.assertEqual(
            len(repr(self.unmasked.__dict__)),
            len(repr({str('name'): str('Bob'), str('age'): 99})))
    ...
    def test_unmasked_age_can_be_deleted_via_dunder_dict(self):
        del self.unmasked.__dict__['age']
        self.assertNotIn('age', self.unmasked.__dict__)
        with self.assertRaisesRegex(KeyError, 'age'):
            self.unmasked.__dict__['age']
        self.assertEqual(
            repr(self.unmasked.__dict__), repr({str('name'): str('Bob')}))
        self.assertEqual(len(self.unmasked.__dict__), 1)

    def test_masked_age_cannot_be_discovered_via_dunder_dict(self):
        self.assertNotIn('age', self.masked.__dict__)
        with self.assertRaisesRegex(MaskedKeyError, 'age'):
            self.masked.__dict__['age']
        self.assertEqual(
            repr(self.masked.__dict__), repr({str('name'): str('Bob')}))
        self.assertEqual(len(self.masked.__dict__), 1)

    def test_masked_age_cannot_be_mutated_via_dunder_dict(self):
        with self.assertRaisesRegex(MaskedKeyError, 'age'):
            self.masked.__dict__['age'] = 99
        self.assertNotIn('age', self.masked.__dict__)
        with self.assertRaisesRegex(MaskedKeyError, 'age'):
            self.masked.__dict__['age']
        self.assertEqual(
    ...
    def test_masked_age_cannot_be_deleted_via_dunder_dict(self):
        with self.assertRaisesRegex(MaskedKeyError, 'age'):
            del self.masked.__dict__['age']
        self.assertNotIn('age', self.masked.__dict__)
        with self.assertRaisesRegex(MaskedKeyError, 'age'):
            self.masked.__dict__['age']
        self.assertEqual(
    ...
    def test_masked_age_not_visible_via_dunder_dict_keys(self):
        self.assertNotIn('age', self.masked.__dict__.keys())

    def test_masked_age_not_visible_via_dunder_dict_values(self):
        self.assertNotIn(42, self.masked.__dict__.values())

    def test_masked_age_not_visible_via_dunder_dict_items(self):
        self.assertNotIn(('age', 42), self.masked.__dict__.items())

    def test_masked_dict_still_tracks_real_dict(self):
        self.assertIs(self.masked.__dict__.data, self.real.__dict__)

    def test_real_age_can_be_accessed(self):
        self.assertEqual(self.real.age, 42)

    def test_unmasked_age_can_be_accessed(self):
        self.assertEqual(self.unmasked.age, 42)

    def test_masked_age_cannot_be_accessed(self):
        with self.assertRaisesRegex(MaskedAttributeError, "age"):
            self.masked.age

    def test_masked_age_cannot_be_assigned(self):
        with self.assertRaises(AttributeError):
            self.masked.age = 1

    def test_masked_age_cannot_be_deleted(self):
        with self.assertRaises(AttributeError):
            del self.masked.age

    def test_masked_age_cannot_be_discovered_via_dir(self):
        self.assertNotIn('age', dir(self.masked))


class proxy_as_function(unittest.TestCase):

    """ Tests for uses of masking_proxy() as factory for proxy objects.  """
    ...
    def setUp(self):
        ...
        self.proxy = masking_proxy(self.obj)
    ...
    def test_descriptor_methods(self):
        ...
        property_proxy = masking_proxy(property)
    ...
    def test_delitem(self):
        ...
        obj = {'k': 'v'}
        del masking_proxy(obj)['k']
        self.assertEqual(obj, {})
        obj = {'k': 'v'}
        masking_proxy(obj).__delitem__('k')
        self.assertEqual(obj, {})
    ...
    def test_add(self):
        ...
        self.assertEqual(masking_proxy(4.5) + 2, 6.5)
        self.assertEqual(masking_proxy(5) + 2, 7)
        with self.assertRaises(TypeError):
            masking_proxy("foo") + 2
        self.assertEqual(masking_proxy("foo") + "bar", "foobar")

    def test_sub(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy - other, self.obj - other)
        self.assertEqual(self.proxy.__sub__(other), self.obj - other)
        self.assertEqual(masking_proxy(4.5) - 2, 2.5)
        self.assertEqual(masking_proxy(5) - 2, 3)
        with self.assertRaises(TypeError):
            masking_proxy("foo") - 2
    ...
    def test_mul(self):
        ...
        self.assertEqual(masking_proxy(4.5) * 2, 9.0)
        self.assertEqual(masking_proxy(5) * 2, 10)
        self.assertEqual(masking_proxy("foo") * 2, "foofoo")
    ...
    def test_div(self):
        ...
        self.assertEqual(self.proxy.__div__(other),
                         operator.div(self.obj, other))
        self.assertEqual(operator.div(masking_proxy(4.5), 2), 2.25)
        self.assertEqual(operator.div(masking_proxy(5), 2), 2)
    ...
    def test_truediv(self):
        ...
        self.assertEqual(self.proxy.__truediv__(other),
                         operator.truediv(self.obj, other))
        self.assertEqual(operator.truediv(masking_proxy(4.5), 2), 2.25)
        self.assertEqual(operator.truediv(masking_proxy(5), 2), 2.5)

    def test_floordiv(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy // other, self.obj // other)
        self.assertEqual(self.proxy.__floordiv__(other), self.obj // other)
        self.assertEqual(masking_proxy(4.5) // 2, 2)
        self.assertEqual(masking_proxy(5) // 2, 2)

    def test_mod(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy % other, self.obj % other)
        self.assertEqual(self.proxy.__mod__(other), self.obj % other)
        self.assertEqual(masking_proxy(4.5) % 2, 0.5)
        self.assertEqual(masking_proxy(5) % 2, 1)
    ...
    def test_divmod(self):
        ...
        self.assertEqual(divmod(masking_proxy(4.5), 2), (2, 0.5))
        self.assertEqual(divmod(masking_proxy(5), 2), (2, 1))
    ...
    def test_pow(self):
        ...
        self.assertEqual(pow(masking_proxy(2), 3), 8)

    def test_lshift(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy << other, self.obj << other)
        self.assertEqual(self.proxy.__lshift__(other), self.obj << other)
        self.assertEqual(masking_proxy(1) << 3, 8)

    def test_rshift(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy >> other, self.obj >> other)
        self.assertEqual(self.proxy.__rshift__(other), self.obj >> other)
        self.assertEqual(masking_proxy(8) >> 3, 1)

    def test_and(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy & other, self.obj & other)
        self.assertEqual(self.proxy.__and__(other), self.obj & other)
        self.assertEqual(masking_proxy(7) & 4, 4)

    def test_xor(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy ^ other, self.obj ^ other)
        self.assertEqual(self.proxy.__xor__(other), self.obj ^ other)
        self.assertEqual(masking_proxy(1) ^ 1, 0)
        self.assertEqual(masking_proxy(1) ^ 0, 1)
        self.assertEqual(masking_proxy(0) ^ 0, 0)
        self.assertEqual(masking_proxy(0) ^ 1, 1)

    def test_or(self):
        other = mock.MagicMock()
        self.assertEqual(self.proxy | other, self.obj | other)
        self.assertEqual(self.proxy.__or__(other), self.obj | other)
        self.assertEqual(masking_proxy(1) | 2, 3)
    ...
    def test_radd(self):
        ...
        self.assertEqual(4.5 + masking_proxy(2), 6.5)
        self.assertEqual(5 + masking_proxy(2), 7)
        with self.assertRaises(TypeError):
            2 + masking_proxy("foo")
        self.assertEqual("foo" + masking_proxy("bar"), "foobar")
        self.assertEqual([1, 2, 3] + masking_proxy([4, 5]), [1, 2, 3, 4, 5])
    ...
    def test_rsub(self):
        ...
        self.assertEqual(4.5 - masking_proxy(2), 2.5)
        self.assertEqual(5 - masking_proxy(2), 3)
        with self.assertRaises(TypeError):
            "foo" - masking_proxy(2)
    ...
    def test_rmul(self):
        ...
        self.assertEqual(4.5 * masking_proxy(2), 9.0)
        self.assertEqual(5 * masking_proxy(2), 10)
        self.assertEqual("foo" * masking_proxy(2), "foofoo")
    ...
    def test_rdiv(self):
        ...
        self.assertEqual(self.proxy.__rdiv__(other),
                         operator.div(other, self.obj))
        self.assertEqual(operator.div(4.5, masking_proxy(2)), 2.25)
        self.assertEqual(operator.div(5, masking_proxy(2)), 2)
    ...
    def test_rtruediv(self):
        ...
        self.assertEqual(self.proxy.__rtruediv__(other),
                         operator.truediv(other, self.obj))
        self.assertEqual(operator.truediv(4.5, masking_proxy(2)), 2.25)
        self.assertEqual(operator.truediv(5, masking_proxy(2)), 2.5)
    ...
    def test_rfloordiv(self):
        ...
        self.assertEqual(4.5 // masking_proxy(2), 2)
        self.assertEqual(5 // masking_proxy(2), 2)
    ...
    def test_rmod(self):
        ...
        self.assertEqual(4.5 % masking_proxy(2), 0.5)
        self.assertEqual(5 % masking_proxy(2), 1)
    ...
    def test_rdivmod(self):
        ...
        self.assertEqual(divmod(4.5, masking_proxy(2)), (2, 0.5))
        self.assertEqual(divmod(5, masking_proxy(2)), (2, 1))
    ...
    def test_rpow(self):
        ...
        self.assertEqual(pow(2, masking_proxy(3)), 8)
        with self.assertRaises(TypeError):
            # __rpow__ is not called for the three-argument version of pow()
            self.assertEqual(pow(2, masking_proxy(10), 1000), 24)
    ...
    def test_rlshift(self):
        ...
        self.assertEqual(1 << masking_proxy(3), 8)
    ...
    def test_rrshift(self):
        ...
        self.assertEqual(8 >> masking_proxy(3), 1)
    ...
    def test_rand(self):
        ...
        self.assertEqual(7 & masking_proxy(4), 4)
    ...
    def test_rxor(self):
        ...
        self.assertEqual(1 ^ masking_proxy(1), 0)
        self.assertEqual(1 ^ masking_proxy(0), 1)
        self.assertEqual(0 ^ masking_proxy(0), 0)
        self.assertEqual(0 ^ masking_proxy(1), 1)
    ...
    def test_ror(self):
        ...
        self.assertEqual(1 | masking_proxy(2), 3)
    ...
    def test_issubclass(self):
        ...
        self.assertTrue(issubclass(str, type(masking_proxy(obj))))
